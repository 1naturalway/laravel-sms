...
class SmsManager implements SmsProvider
{
    ...
    protected function resolve(string $name): SmsProvider
    {
        ...
        return match ($name) {
            ...
            'smstrap' => $this->createSmsTrapDriver($config),
    ...
    /**
     * Create an instance of the SmsTrap driver.
     *
     * @param  array<string, mixed>  $config
     */
    protected function createSmsTrapDriver(array $config): SmsTrapProvider
    {
        return new SmsTrapProvider(
            url: $config['url'] ?? '',
